...
const polarCoordinates = ([r, theta]) =>
  realNumbers(r) && -Math.PI <= theta && theta <= Math.PI;

const realMetric = (x1, x2) => Math.abs(x2 - x1);
const cartesianMetric = ([x1, y1], [x2, y2]) =>
  Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2);
const bunchedIntegerMetric = (x1, x2) =>
  x1 % 2 !== 0 || x2 % 2 !== 0
    ? Math.abs(x2 - x1) / 2 + 1
    : Math.abs(x2 - x1) / 2;
const taxicabMetric = ([x1, y1], [x2, y2]) =>
  Math.abs(x2 - x1) + Math.abs(y2 - y1);
const polarMetric = ([r1, t1], [r2, t2]) =>
  Math.sqrt(r2 ** 2 + r1 ** 2 - 2 * r1 * r2 * Math.cos(t2 - t1));
