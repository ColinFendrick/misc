const typecheck = type => vals => !(vals.map(v => typeof v === type).includes(false));
...
const applyToSubspace = ({
	...
	vals.filter(x => numcheck(x) && x.map(domain)).map(val => ([val, metric(val)]))
		.filter(([, y]) => range(y));

const realClosedSubset = ([r1, r2]) => x => (r1 <= x) && (x <= r2);
const logarithmicMetric = () => ([x1, x2]) => x1 && x2 ? Math.log(x1) * Math.log(x2) : 0;
